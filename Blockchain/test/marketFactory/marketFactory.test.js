...
describe("Market Factory test", async () => {
    ...
    describe("Admin Managed functions", async () => {
        ...
        it("Only mol admin can remove an admin", async () =>{
            ...
            await assert.revert(marketFactoryInstance.from(user1).removeWhitelistAdmin(molAdmin.signer.address))
            // Checks the super admin can remove another admin
            await assert.notRevert(marketFactoryInstance.from(molAdmin).removeWhitelistAdmin(user1.signer.address))
        });
        ...
        it("One admin must always be present in the contract", async () =>{
            ...
            let adminCount = await marketFactoryInstance.getAdminCount();
            assert.equal(
                adminCount.toString(),
                1,
            ...
            await assert.revert(marketFactoryInstance.from(molAdmin).renounceWhitelistAdmin());
            // Checking there is still the correct number of admins in the contract
            adminCount = await marketFactoryInstance.getAdminCount();
            assert.equal(
                adminCount.toString(),
                1,
                "Incorrect admin count"
