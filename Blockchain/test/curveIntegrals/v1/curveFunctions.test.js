...
describe("Curve Functions test", async () => {
  let molAdmin = accounts[1];
  let creator = accounts[2];
  let user1 = accounts[3];
  let user2 = accounts[4];
  let pseudoDaiInstance,
    moleculeVaultInstance,
    curveRegistryInstance,
    marketRegistryInstance,
    marketFactoryInstance,
    curveIntegralInstance;

  beforeEach("", async () => {
    deployer = new etherlime.EtherlimeGanacheDeployer(molAdmin.secretKey);

    curveFunctionsInstance = await deployer.deploy(CurveFunctionsAbi, false);
  });

  describe("Curve checks", async () => {
    it("Curve Integral functions as expected", async () => {
      const rawDaiBN = await curveFunctionsInstance
        .from(molAdmin)
    ...
    it("Inverse Curve Integral functions as expected", async () => {
      const rawDaiBN = await curveFunctionsInstance
        .from(molAdmin)
        .curveIntegral(simulatedCurve.defaultPurchaseAmount);
      const rawTokenBN = await curveFunctionsInstance
        .from(molAdmin)
