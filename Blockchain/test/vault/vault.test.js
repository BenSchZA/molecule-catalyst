...
describe('Vault test', () => {
    ...
    describe("Market interactions", () => {
        it("Validates funding on mint", async () => {
            let currentPhase = await vaultInstance.currentPhase();
            assert.ok(currentPhase.eq(0), "Phase invalid");

            const estimateTokens = await marketInstance.colateralToTokenBuying(ethers.utils.parseUnits("9500000", 18))
            ...
            console.log(ethers.utils.formatUnits(currentPhase, 18))
